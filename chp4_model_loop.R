modelSpecies = function(inRow){
  ...
  if(!is.na(inRow[["w.pa.sampling.covs"]])){names.w.pa.sampling.covs = strsplit(inRow[["w.pa.sampling.covs"]],",")[[1]]} else{names.w.pa.sampling.covs=NULL}
  
  names_allCovs = unique(c(names.x.po.covs,names.w.po.covs,names.x.pa.covs,names.w.pa.covs))
  
  ### prep data
  
  allStack.files = list.files(path=paste0("/Users/codyschank/Dropbox/Research/Dissertation/dataPrep_output/",resolution,"km_chp4/"),full.names=TRUE)
...
inputs = read.csv("/Users/codyschank/Dropbox/Research/Dissertation/inputs_chp4.csv",stringsAsFactors=FALSE)
...
outputFolder.base = paste0("/Users/codyschank/Dropbox/Research/Dissertation/outputs_chp4_",resolutions,"km_final3/")
...
if(length(inputs$w.pa.sampling.covs[!is.na(inputs$w.pa.sampling.covs)])>0){
  inputs[,paste('w.pa.sampling.',unique(unlist(strsplit(inputs$w.pa.sampling.covs,","))),sep='')] <- NA
  inputs[,paste('w.pa.sampling.',unique(unlist(strsplit(inputs$w.pa.sampling.covs,","))),'.se',sep='')] <- NA
}

inputs[,c("model","converged","estimated.se","r.po","r.pa","n.po","n.pa","conv","recip","AIC","DIC","value","deviance",
